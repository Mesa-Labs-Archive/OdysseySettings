...
package com.odyssey.settings.act;
...
/*        if(mImageName!=null) {
    ...
    private void start_img_picker(){
        ...
        if(just_get_uri_mode) startActivityForResult(Intent.createChooser(intent, getString(R.string.gs_selecc_image_usando)),REQ_COD_JUST_GET_URI);
        else startActivityForResult(Intent.createChooser(intent, getString(R.string.gs_selecc_image_usando)),REQ_COD_MODO_IMG);
    }
    ...
    private boolean do_circle_img(){
        ...
        if(!resultado) Toast.makeText(this,getString(R.string.gs_error_redondeando),Toast.LENGTH_SHORT).show();
        return resultado;
    }
}
